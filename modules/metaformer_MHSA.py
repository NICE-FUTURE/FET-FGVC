""" copied from MetaFormer: https://github.com/dqshuai/MetaFormer
"""
...
from network.DSwin import fastMlp
...
class Relative_Attention(nn.Module):
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * img_size[0] - 1) * (2 * img_size[1] - 1) + 1, num_heads)
        )  # 2*h-1 * 2*w-1 + 1, nH
        ...
        relative_position_index = F.pad(
            relative_position_index, (extra_token_num, 0, extra_token_num, 0)
        )
...
class MHSABlock(nn.Module):
    def __init__(
        ...
        if stride != 1:
            self.patch_embed = OverlapPatchEmbed(
                patch_size=3, stride=stride, in_chans=input_dim, embed_dim=output_dim
            )
    ...
    def forward(self, x, extra_tokens=None):
        ...
        shortcut = x
        x, v, attn = self.attn(self.norm1(x))
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, v, attn


class AdaMHSABlock(nn.Module):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        self.fastmlp = fastMlp(in_features=output_dim, act_layer=act_layer, drop=drop)

    def forward(self, x, extra_tokens=None, mask=None):
        if self.patch_embed is not None:
            x, _, _ = self.patch_embed(x)
