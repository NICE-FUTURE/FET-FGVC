...
class BasicLayer(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., norm_layer=nn.LayerNorm, downsample=None, sparse_ratio=[], pruning_loc=[]):
        ...
        assert len(sparse_ratio) == len(pruning_loc), "len(sparse_ratio) != len(pruning_loc)"
    ...
    def forward(self, x):
        ...
        if len(self.sparse_ratio) == 0:
            ...
            # if len(x) == 2:  # fixed: when batch size is 2, it can not determine training or validating stage
            if self.training:
...
class AdaSwinTransformer(nn.Module):
    ...
    def __init__(
            ...
            sparse_ratio=[0.9,0.7,0.5], pruning_loc=[4,8,12], **kwargs):
        super().__init__()
        ...
        layers[i] = BasicLayer(
            ...
            norm_layer=norm_layer,
            downsample=PatchMerging if (i < self.num_layers - 1) else None,
            sparse_ratio=sparse_ratio, pruning_loc=pruning_loc
        )
